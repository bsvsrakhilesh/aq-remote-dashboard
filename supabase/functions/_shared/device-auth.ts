...
export async function authenticateDevice(request: Request) {
  const code = request.headers.get('x-device-id'); const secret = request.headers.get('x-device-key')
  if (!code || !secret || secret.length < 24) return null
  const db = adminClient(); const { data } = await db.rpc('verify_device_secret', { p_device_code: code, p_secret: secret }).single()
