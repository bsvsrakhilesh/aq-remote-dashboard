...
// Keep the device key and IITD username outside the shareable sketch.
// private_config.h must stay beside this .ino when compiling.
#if __has_include("private_config.h")
#include "private_config.h"
#endif
#ifndef INDOOR_DEVICE_SECRET
#define INDOOR_DEVICE_SECRET "YOUR_DEVICE_SECRET"
#endif
#ifndef INDOOR_IITD_USERNAME
#define INDOOR_IITD_USERNAME "YOUR_IITD_USERNAME"
#endif
...
// Shared I2C bus: XIAO Expansion Board onboard OLED + SCD30 + SPS30 + DS3231
#define I2C_SDA      D4
#define I2C_SCL      D5
#define OLED_RESET   -1
...
// For multiple units, use aq01, aq02, aq03, ...
// DHCP may change the numeric IP, but mDNS lets you use:
// http://aq01.local
const char* DEVICE_HOSTNAME = "aq_indoor01";
const char* NETWORK_HOSTNAME = "aq-indoor01";

// Firmware identifier shown in the remote dashboard heartbeat.
const char* FIRMWARE_VERSION = "pm-co2-iitd-cloud-1.2.1";

// -----------------------------------------------------------------------------
// IIT Delhi WPA2-Enterprise / PEAP configuration
// -----------------------------------------------------------------------------
// Set the username in private_config.h. Do not paste your Kerberos password
// into source code. Serial asks for it and stores it after a successful login.
const char* IITD_WIFI_SSID = "IITD_WIFI";
const char* IITD_EAP_IDENTITY = INDOOR_IITD_USERNAME;
const char* IITD_EAP_USERNAME = INDOOR_IITD_USERNAME;
...
const char* CLOUD_DEVICE_ID = "aq_indoor01";
const char* CLOUD_DEVICE_SECRET = INDOOR_DEVICE_SECRET;
...
const char* CLOUD_SUPABASE_ANON_KEY = "sb_publishable_1v2DeCX-1ayasTpo9N-HSA_3w1-yVVI";
...
// Example:
// aq_indoor_2026-09-14_to_2026-09-20.csv
...
// SCD30 documented continuous-measurement interval range: 2-1800 s.
const unsigned long SCD30_MIN_INTERVAL_MS = 2000UL;
const unsigned long SCD30_MAX_INTERVAL_MS = 1800000UL;
...
const char* INTERVAL_PREF_NAMESPACE = "intervalsV2";
...
float co2Concentration = NAN;
float temperature      = NAN;
float humidity         = NAN;
int16_t scd30Error     = 0;
char scd30ErrorMsg[128];

bool sps30HasValidData = false;
bool scd30HasValidData = false;
...
String scd30Health = "NOT CHECKED";
...
// user-selected SPS30/SCD30 sampling interval.
unsigned long lastSps30PollMs = 0;
unsigned long lastScd30PollMs = 0;
unsigned long lastSps30SampleMs = 0;
unsigned long lastScd30SampleMs = 0;
unsigned long lastLogMs = 0;

const char* WEEKLY_CSV_HEADER =
    "Date,Time,MC1.0,MC2.5,MC4.0,MC10.0,NC0.5,NC1.0,NC2.5,NC4.0,NC10.0,ParticleSize,CO2,Temp,Humidity";
const char* RECOVERY_CSV_HEADER =
    "Record,UptimeMs,Date,Time,MC1.0,MC2.5,MC4.0,MC10.0,NC0.5,NC1.0,NC2.5,NC4.0,NC10.0,ParticleSize,CO2,Temp,Humidity";
...
void scd30PrintError(const char* prefix, int16_t error) {
  Serial.print(prefix);
  errorToString(error, scd30ErrorMsg, sizeof(scd30ErrorMsg));
  Serial.println(scd30ErrorMsg);
}

unsigned long normalizeScd30IntervalMs(unsigned long requestedMs) {
  if (requestedMs < SCD30_MIN_INTERVAL_MS) {
    requestedMs = SCD30_MIN_INTERVAL_MS;
  }
  if (requestedMs > SCD30_MAX_INTERVAL_MS) {
    requestedMs = SCD30_MAX_INTERVAL_MS;
  }

  // SCD30 accepts whole seconds. Round to the nearest whole second.
  unsigned long seconds = (requestedMs + 500UL) / 1000UL;
  if (seconds < 2UL) seconds = 2UL;
  if (seconds > 1800UL) seconds = 1800UL;
  return seconds * 1000UL;
}

int16_t applyScd30MeasurementInterval() {
  SCD30_READ_INTERVAL = normalizeScd30IntervalMs(SCD30_READ_INTERVAL);
  uint16_t seconds = (uint16_t)(SCD30_READ_INTERVAL / 1000UL);
  return scd30.setMeasurementInterval(seconds);
}
...
bool scd30DataUsable(unsigned long nowMs) {
  if (!scd30HasValidData) return false;
  return (nowMs - lastScd30SampleMs) <= sensorStaleLimitMs(SCD30_READ_INTERVAL);
}
...
void handleRoot() {
  ...
  html += F("<div class='healthitem'><b>SCD30</b><br><span id='scdHealth'>--</span></div>");
  ...
  html += F("<div class='card'><h2>SCD30 Data</h2>");
  html += "<p id='scdStatus'>--</p>";
  html += F("<table><tr><th>Parameter</th><th>Value</th></tr>");
  html += "<tr><td>CO2 (ppm)</td><td id='co2'>--</td></tr>";
  ...
  html += F("<p><a class='btn' href='/frc'>Force Recalibration (FRC)</a></p>");
...
async function refreshLiveData() {
  ...
  try {
    ...
    putNumber('co2', d.co2, 1); putNumber('temp', d.temp, 2); putNumber('rh', d.rh, 2);
    ...
    setHealth('scdHealth', d.scdHealth, d.scdValid, d.scdInitialized);
    ...
    const allRequired = d.oledOk && d.timeVerified && d.spsValid && d.scdValid && d.sdHealthy;
    ...
    const scdStatus = document.getElementById('scdStatus');
    scdStatus.textContent = d.scdHealth + (d.scdAgeMs !== null ? ' | sample age ' + Math.round(d.scdAgeMs/1000) + ' s' : '');
    scdStatus.className = d.scdValid ? 'ok' : 'err';
...
void handleLiveData() {
  ...
  bool scdLiveUsable = scd30DataUsable(liveMs);

  if (sps30Initialized && !spsLiveUsable && sps30HasValidData) sps30Health = "STALE DATA";
  if (scd30Initialized && !scdLiveUsable && scd30HasValidData) scd30Health = "STALE DATA";
  if (sps30Initialized && !sps30HasValidData && sps30Health == "OK") sps30Health = "WAITING FOR DATA";
  if (scd30Initialized && !scd30HasValidData && scd30Health == "OK") scd30Health = "WAITING FOR DATA";
  DateTime trustedNow;
  ...
  else if (!scd30Initialized) hint = "SCD30 not detected. Check SCD30 power, SDA/SCL and address.";
  else if (!scdLiveUsable) hint = "SCD30 is connected but data is missing/stale. Check I2C connection and sensor warm-up.";
  ...
  json += F(",\"scdValid\":"); json += scdLiveUsable ? F("true") : F("false");
  ...
  json += F(",\"scdInitialized\":"); json += scd30Initialized ? F("true") : F("false");
  ...
  json += F(",\"scdHealth\":\""); json += scd30Health; json += F("\"");
  ...
  json += F(",\"co2\":"); json += jsonFloatOrNull(co2Concentration, scdLiveUsable);
  json += F(",\"temp\":"); json += jsonFloatOrNull(temperature, scdLiveUsable);
  json += F(",\"rh\":"); json += jsonFloatOrNull(humidity, scdLiveUsable);
  json += F(",\"spsAgeMs\":"); json += sps30HasValidData ? String(liveMs-lastSps30SampleMs) : String("null");
  json += F(",\"scdAgeMs\":"); json += scd30HasValidData ? String(liveMs-lastScd30SampleMs) : String("null");
...
void handleFRC() {
  ...
  html += F("<div class='card'><h2>Force Recalibration (FRC)</h2>");
  html += F("<p>Use only a known stable CO2 reference. Allowed SCD30 reference range: 400-2000 ppm.</p>");

  uint16_t frcValue = 0;
  int16_t err = scd30.getForceRecalibrationStatus(frcValue);
  if (err == NO_ERROR) {
    html += "<p>Current FRC reference value: " + String(frcValue) + " ppm</p>";
  } else {
    errorToString(err, scd30ErrorMsg, sizeof(scd30ErrorMsg));
    html += "<p class='err'>Could not read FRC reference: " + String(scd30ErrorMsg) + "</p>";
  }
  ...
  if (server.hasArg("ref")) {
    ...
    if (ref < 400 || ref > 2000) {
      html += F("<p class='err'>Reference must be between 400 and 2000 ppm.</p>");
    } else {
      scd30Error = scd30.forceRecalibration((uint16_t)ref);
      if (scd30Error == NO_ERROR) {
        html += "<p class='ok'>FRC applied successfully using " + String(ref) + " ppm.</p>";
      } else {
        errorToString(scd30Error, scd30ErrorMsg, sizeof(scd30ErrorMsg));
        html += "<p class='err'>FRC failed: " + String(scd30ErrorMsg) + "</p>";
      }
    }
  }

  html += F(
      "<hr><form method='get' action='/frc'>"
      "<label for='ref'>CO2 reference (ppm):</label><br>"
      "<input type='number' id='ref' name='ref' min='400' max='2000' required>"
      "<br><br><input class='btn' type='submit' value='Force Recalibrate'>"
      "</form></div>");
  html += F("<p><a class='btn' href='/'>Back to Main</a></p>");
  html += getHtmlFooter();
  server.send(200, "text/html", html);
...
void handleTempOffset() {
  String html = getHtmlHeader("SCD30 Temperature Offset");

  uint16_t currentOffsetRaw = 0;
  int16_t err = scd30.getTemperatureOffset(currentOffsetRaw);
  html += F("<div class='card'><h2>Current Temperature Offset</h2>");
  if (err == NO_ERROR) {
    float currentOffsetC = currentOffsetRaw / 100.0f;
    html += "<p>" + String(currentOffsetC, 2) + " degC</p>";
  } else {
    errorToString(err, scd30ErrorMsg, sizeof(scd30ErrorMsg));
    html += "<p class='err'>Error reading offset: " + String(scd30ErrorMsg) + "</p>";
  }
  html += F("</div>");

  if (server.hasArg("offset")) {
    float requestedOffsetC = server.arg("offset").toFloat();
    html += F("<div class='card'><h2>Apply Offset</h2>");

    // Raw field is uint16_t in units of degC x 100.
    if (!isfinite(requestedOffsetC) || requestedOffsetC < 0.0f || requestedOffsetC > 655.35f) {
      html += F("<p class='err'>Offset must be between 0.00 and 655.35 degC.</p>");
    } else {
      uint16_t rawOffset = (uint16_t)lroundf(requestedOffsetC * 100.0f);
      err = scd30.setTemperatureOffset(rawOffset);
      if (err == NO_ERROR) {
        html += "<p class='ok'>Set to " + String(requestedOffsetC, 2) + " degC successfully.</p>";
      } else {
        errorToString(err, scd30ErrorMsg, sizeof(scd30ErrorMsg));
        html += "<p class='err'>Failed: " + String(scd30ErrorMsg) + "</p>";
      }
    }
    html += F("</div>");
  }
  ...
  html += F(
      "<div class='card'><h2>Enter New Offset (degC)</h2>"
      "<form method='get' action='/tempOffset'>"
      "<div class='row'><input type='number' step='0.01' min='0' max='655.35' name='offset' required></div>"
...
void loadIntervalsFromPrefs() {
  ...
  SCD30_PAGE_INTERVAL = intervalPrefs.getULong("scdPage", 6000UL);
  IP_PAGE_INTERVAL    = intervalPrefs.getULong("ipPage", 3000UL);
  SPS30_READ_INTERVAL = intervalPrefs.getULong("spsRead", 2000UL);
  SCD30_READ_INTERVAL = intervalPrefs.getULong("scdRead", 2000UL);
  ...
  SCD30_PAGE_INTERVAL = max(SCD30_PAGE_INTERVAL, DISPLAY_MIN_INTERVAL_MS);
...
void saveIntervalsToPrefs() {
  ...
  intervalPrefs.putULong("scdPage", SCD30_PAGE_INTERVAL);
  intervalPrefs.putULong("ipPage", IP_PAGE_INTERVAL);
  intervalPrefs.putULong("spsRead", SPS30_READ_INTERVAL);
  intervalPrefs.putULong("scdRead", SCD30_READ_INTERVAL);
...
void handleConfig() {
  ...
  if (server.hasArg("pmPage")) {
    ...
    unsigned long newScdPage = max(webArgULong("scdPage", SCD30_PAGE_INTERVAL), DISPLAY_MIN_INTERVAL_MS);
    ...
    unsigned long newScdRead = normalizeScd30IntervalMs(webArgULong("scdRead", SCD30_READ_INTERVAL));
    unsigned long newLog     = max(webArgULong("log", LOG_INTERVAL), LOG_MIN_INTERVAL_MS);

    // Apply the SCD30 hardware interval before committing it to Preferences.
    unsigned long oldScdRead = SCD30_READ_INTERVAL;
    SCD30_READ_INTERVAL = newScdRead;
    int16_t err = applyScd30MeasurementInterval();

    if (err != NO_ERROR) {
      SCD30_READ_INTERVAL = oldScdRead;
      errorToString(err, scd30ErrorMsg, sizeof(scd30ErrorMsg));
      statusMessage = "<p class='err'>SCD30 interval was not changed: " + String(scd30ErrorMsg) + "</p>";
    } else {
      PM_PAGE_INTERVAL = newPmPage;
      NC_PAGE_INTERVAL = newNcPage;
      SCD30_PAGE_INTERVAL = newScdPage;
      IP_PAGE_INTERVAL = newIpPage;
      SPS30_READ_INTERVAL = newSpsRead;
      LOG_INTERVAL = newLog;

      // Restart timing anchors so changes take effect cleanly.
      lastSps30PollMs = 0;
      lastScd30PollMs = 0;
      lastLogMs = millis();
  ...
  html += "<div class='row'><label>SCD30 Page</label><input name='scdPage' type='number' min='500' step='100' value='" + String(SCD30_PAGE_INTERVAL) + "' required></div>";
  ...
  html += "<div class='row'><label>SCD30 Read</label><input name='scdRead' type='number' min='2000' max='1800000' step='1000' value='" + String(SCD30_READ_INTERVAL) + "' required></div>";
  ...
  html += F("<p>SCD30 accepts whole-second measurement intervals from 2000 to 1800000 ms.</p>");
  html += F("</div>");
...
bool tryInitScd30() {
  ...
  // A firmware-version read is used as the presence check.
  uint8_t major = 0;
  uint8_t minor = 0;
  scd30Error = scd30.readFirmwareVersion(major, minor);
  if (scd30Error != NO_ERROR) {
    scd30Initialized = false;
    scd30Health = "NOT FOUND";
    return false;
  }

  // Configure the requested SCD30 measurement interval, then ensure
  // continuous measurement is running.
  scd30.stopPeriodicMeasurement();
  delay(5);

  scd30Error = applyScd30MeasurementInterval();
  if (scd30Error != NO_ERROR) {
    scd30Initialized = false;
    scd30Health = "CONFIG FAILED";
    return false;
  }

  scd30Error = scd30.startPeriodicMeasurement(0);
  if (scd30Error != NO_ERROR) {
    scd30Initialized = false;
    scd30Health = "START FAILED";
    return false;
  }

  scd30Initialized = true;
  scd30Health = scd30HasValidData ? "OK" : "WAITING FOR DATA";
  return true;
}
...
void serviceComponentRecovery() {
  ...
  if (!scd30Initialized) tryInitScd30();
...
void serviceScd30() {
  if (!scd30Initialized) return;
  unsigned long ms = millis();

  // SCD30_READ_INTERVAL configures the sensor's INTERNAL measurement cadence.
  // The ESP32 polls the ready flag much more frequently so it reads each new
  // measurement shortly after it becomes available, regardless of phase drift
  // between the SCD30 clock and the SD logging clock.
  if (ms - lastScd30PollMs < SENSOR_READY_POLL_INTERVAL_MS) return;
  lastScd30PollMs = ms;

  uint16_t dataReady = 0;
  scd30Error = scd30.getDataReady(dataReady);
  if (scd30Error != NO_ERROR) {
    scd30PrintError("SCD30 getDataReady error: ", scd30Error);
    return;
  }
  ...
  scd30Error = scd30.readMeasurementData(newCo2, newTemp, newRh);
  if (scd30Error != NO_ERROR) {
    scd30PrintError("SCD30 readMeasurementData error: ", scd30Error);
    return;
  }

  if (!isfinite(newCo2) || !isfinite(newTemp) || !isfinite(newRh)) {
    Serial.println("SCD30 returned non-finite data; sample ignored.");
    return;
  }
  ...
  temperature = newTemp;
  humidity = newRh;
  scd30HasValidData = true;
  scd30Initialized = true;
  scd30Health = "OK";
  lastScd30SampleMs = ms;
}
...
String firstDiagnosticFault(unsigned long ms) {
  ...
  if (!scd30Initialized) return "SCD30 NOT FOUND";
  if (!scd30DataUsable(ms)) return "SCD30 NO DATA";
...
unsigned long currentPageDuration() {
  switch (currentDisplay) {
    ...
    case SHOW_SCD30: return SCD30_PAGE_INTERVAL;
...
void updateOledIfNeeded() {
  ...
  if (fault.length() > 0) {
    ...
    } else if (!scd30Initialized || !scd30DataUsable(ms)) {
      display.print("Check SCD30 power/I2C");
    } else {
  ...
  switch (currentDisplay) {
    ...
    case SHOW_SCD30:
      ...
      if (!scd30HasValidData) {
        display.println("Waiting for SCD30");
      } else {
        display.print("CO2 : ");
        display.print(co2Concentration, 1);
        display.println(" ppm");
...
void serviceSdLogging() {
  ...
  bool scdUsable = scd30DataUsable(ms);
  ...
  float co2  = scdUsable ? co2Concentration : NAN;
  float temp = scdUsable ? temperature : NAN;
  float rh   = scdUsable ? humidity : NAN;

  char values[256];
  int valueLength = snprintf(
      values, sizeof(values),
      "%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",
      pm1, pm25, pm4, pm10, nc05, nc1, nc25, nc4, nc10, psize, co2, temp, rh);
...
void sendCloudHeartbeat() {
  ...
  body += ",\"sht3x_ok\":false";  // This logger uses SCD30, not SHT3x.
...
void sendCloudTelemetry() {
  ...
  body += ",\"temperature\":" + jsonFloatOrNull(temperature, scdOk);
  body += ",\"rh\":" + jsonFloatOrNull(humidity, scdOk);
...
void setup() {
  ...
  delay(200);
  Serial.println("Starting diagnostic PM + CO2 data logger...");
  ...
  if (tryInitScd30()) {
    showStartupScreen("SCD30 FOUND", "Waiting for data...");
  } else {
    showStartupScreen("SCD30 NOT FOUND", "Will retry in loop");
  }
