...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (demoMode || readOnlyMode) return children
  ...
  if (loading)
    return (
      <div className="auth-loading">
        <Activity className="spin" /> Establishing secure session…
      </div>
    )
    ...
    <main className="login-page">
      <section className="login-story">
        ...
        <div>
          ...
          <p>Secure remote visibility for research-grade air quality logger networks.</p>
        </div>
        <small>Full-resolution data stays on the logger. Always.</small>
      </section>
      <section className="login-form-wrap">
        <form className="login-card" onSubmit={submit}>
          ...
          <h2>Welcome back</h2>
          <p>Sign in to monitor your logger network.</p>
          <label>
            Email address
            <input
              type="email"
