...
try {
  for (const theme of ['light', 'dark']) {
    for (const width of [1440, 768, 390, 320]) {
      ...
      assert.equal(await page.locator('.metrics .metric-card').count(), 6)
      assert.equal(await page.locator('.metrics .metric-card').first().getAttribute('aria-label'), 'PM1')
      ...
      const readings = page.locator('.device-readings')
      for (const group of await readings.all()) {
        const labels = await group.locator('small').allTextContents()
        const hasCo2 = (await group.getAttribute('class')).includes('has-co2')
        assert.deepEqual(labels, ['PM1', 'PM2.5', 'PM10', 'Temp.', 'Humidity', ...(hasCo2 ? ['CO₂'] : [])])
        const sizes = await group
          .locator('div')
          .evaluateAll((tiles) => tiles.map((tile) => tile.getBoundingClientRect().height))
        assert.ok(sizes.every((height) => Math.abs(height - sizes[0]) < 1))
      }
      assert.equal(await page.locator('.fleet-co2').count(), 0)
      await page.screenshot({ path: `.smoke/fleet-readings-${theme}-${width}.png`, fullPage: true })
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false)
